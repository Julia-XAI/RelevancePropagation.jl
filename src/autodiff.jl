...
"""
# NOTE: the batched fallback aborts Julia on boxed-scalar adjoints,
# e.g. LayerNorm's (see the `thunk_vjp2` warning) —
# keep new consumers of this function gated to weight-carrying layers.
# A safe per-layer opt-out is one method built from
# `prepare_vjp` + `seeded_pullback` (two width-1 tapes).
